...
class Payone_Core_Block_Adminhtml_System_Config_Form_Field_RatePayDirectDebitStoreIds
    extends Payone_Core_Block_Adminhtml_System_Config_Form_Field_Abstract
{
    ...
    {
        parent::__construct();
        $this->setTemplate('payone/core/system/config/form/field/ratepay_direct_debit_shopids.phtml');
    }

    protected function _prepareToRender()
    {
        $this->addColumn(
            'ratepay_shopid', array(
            'label' => Mage::helper('payone_core')->__('Shop-ID'),
            'style' => 'width:60px;',
            )
        );
        $this->addColumn(
            'ratepay_currency', array(
